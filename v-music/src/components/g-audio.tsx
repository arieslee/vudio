...
export default function GAudio() {
  ...
  const hiddeImgInput = useRef<HTMLInputElement>(null)
...
function handleLoadCoverImage(){
  hiddeImgInput.current?.click()
}
function handleFileCoverImageChange(e: any) {
  ...
  function handleChooseLocalMusic() {
      ...
      <main className={style.page}>
        <div className="ctl-wrapper">
          <div className={style.operationWrapper}>
            <button className="btn m10" onClick={handleLoadCoverImage}>Choose a Cover Image</button>
            <button className="btn m10" onClick={handleChooseLocalMusic}>Choose a local muisc</button>
            <div className="strong-text m10" style={{minWidth: '200px'}}>{musicName}</div>
            <input type="file" style={{display: 'none'}} ref={hiddeImgInput} onChange={handleFileCoverImageChange} />
